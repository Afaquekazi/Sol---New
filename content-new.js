...
let isWorkflowMinimized = false;
let activeWorkflowExecutor = null;
let workflowUIState = null;
...
function setShadowRoot(value) { shadowRoot = value; }
function setButton(value) { button = value; }
function setOutputText(value) { outputText = value; }
function setSelectedMode(value) { selectedMode = value; }
function setSolthronContainer(value) { solthronContainer = value; }
function setIsButtonVisible(value) { isButtonVisible = value; }

function setCurrentCategory(value) { currentCategory = value; }
function setActivePromptId(value) { activePromptId = value; }
function setIsPromptStarActive(value) { isPromptStarActive = value; }

function setActiveChatId(value) { activeChatId = value; }
function setIsChatStarActive(value) { isChatStarActive = value; }

function setActiveWorkflow(value) { activeWorkflow = value; }
function setActiveWorkflowName(value) { activeWorkflowName = value; }
function setIsWorkflowExecuting(value) { isWorkflowExecuting = value; }
function setIsWorkflowMinimized(value) { isWorkflowMinimized = value; }
function setActiveWorkflowExecutor(value) { activeWorkflowExecutor = value; }
function setWorkflowUIState(value) { workflowUIState = value; }

function setRightClickFeaturesEnabled(value) { rightClickFeaturesEnabled = value; }
function setLastSelectedRightClickMode(value) { lastSelectedRightClickMode = value; }
function setPageCredits(value) { pageCredits = value; }

function setAutocompleteDropdown(value) { autocompleteDropdown = value; }
function setAutocompleteMatches(value) { autocompleteMatches = value; }
function setSelectedAutocompleteIndex(value) { selectedAutocompleteIndex = value; }
function setLastAtPosition(value) { lastAtPosition = value; }
function setCurrentInputField(value) { currentInputField = value; }

function setJustSavedPrompt(value) { justSavedPrompt = value; }
function setExtensionSessionId(value) { extensionSessionId = value; }


// ========== BACKEND-AUTH.JS (MODULAR) ==========
...
async function exchangeFirebaseTokenForBackendJWT(firebaseToken) {
    try {
...
async function deductCredits(feature) {
    try {
        const token = await getAuthToken();
        if (!token) {
...
function getFeatureCredits(mode) {
    ...
    const aiAssistantModes = ['smart_followups']; // 5 credits
    const imageModes = ['image_prompt']; // 5 credits
    const freeModes = ['save_note', 'save_persona'];

    // New credit-based features (2 credits each)
    const twoCreditsFeatures = ['workflow_step', 'save_prompt', 'save_response', 'prompt_autocomplete'];

    if (explainModes.includes(mode)) return 5;
    if (aiAssistantModes.includes(mode)) return 5; // Updated from 15 to 5
    if (imageModes.includes(mode)) return 5; // Updated from 12 to 5
    if (twoCreditsFeatures.includes(mode)) return 2; // New: 2 credits
...
function initializeAutocomplete() {
    // Function to find and monitor AI platform input fields
    function findAndMonitorInputFields() {
        const platform = detectAIPlatform();
        ...
        for (const selector of selectors) {
            ...
            inputs.forEach(input => {
                ...
                if (input.hasAttribute('data-solthron-autocomplete')) {
                    return;
                }

                // Mark as initialized
                input.setAttribute('data-solthron-autocomplete', 'true');

                // Add event listeners
                input.addEventListener('input', handleAutocompleteInput);
                input.addEventListener('keydown', handleAutocompleteKeydown);

                // Track current input field on focus
                input.addEventListener('focus', () => {
                    currentInputField = input;
                });

                // Clear on blur (with delay to allow dropdown clicks)
                input.addEventListener('blur', () => {
                    setTimeout(() => {
                        if (document.activeElement !== input) {
                            // Don't clear if clicking dropdown
                            const isClickingDropdown = autocompleteDropdown &&
            ...
            if (inputs.length > 0) {
                break; // Found inputs, no need to try other selectors
            }
        }
    }

    // Initial search
    findAndMonitorInputFields();

    // Use MutationObserver to detect new input fields (for dynamic content)
...
function updateGalleryHeaderWithAddButton() {
    ...
    // ✅ FIX: Remove any existing wrapper/button before creating new ones
    const existingWrapper = shadowRoot.getElementById('prompt-buttons-wrapper');
    const existingButton = shadowRoot.getElementById('add-prompt-btn');
    ...
    // Check if we're in prompts view
    if (currentCategory === 'prompts') {
        ...
        rightButtonsWrapper.id = 'prompt-buttons-wrapper'; // Add ID for easier cleanup
...
function updateGalleryHeaderWithAddWorkflowButton() {
    ...
    // ✅ FIX: Remove any existing wrapper/button before creating new ones
    const existingWrapper = shadowRoot.getElementById('workflow-buttons-wrapper');
    const existingButton = shadowRoot.getElementById('add-workflow-btn');
    ...
    // Check if we're in workflows view
    if (currentCategory === 'workflows') {
        ...
        // Create a wrapper for right-side buttons (+ icon and back button)
        const rightButtonsWrapper = document.createElement('div');
        rightButtonsWrapper.id = 'workflow-buttons-wrapper'; // Add ID for easier cleanup
        rightButtonsWrapper.style.cssText = `
            display: flex;
            align-items: center;
            gap: 12px;
...
class WorkflowExecutor {
    ...
    async executeAll() {
        ...
        // Show minimize button and hide copy button
    ...
    async executeStep(stepIndex) {
        ...
        // ✅ Check credits before executing step
        const creditCheck = await checkCredits('workflow_step');
        if (!creditCheck.success) {
            showNotification(creditCheck.message || 'Insufficient credits for workflow step', 'error');
        ...
        try {
            ...
            // ✅ Deduct credits after successful step execution
            const deductResult = await BackendAuth.deductCredits('workflow_step');
            if (deductResult.success) {
                pageCredits = deductResult.remainingCredits;
            }
    ...
    buildPromptWithContext(step, stepIndex) {
        ...
        // ✅ DEFENSIVE: Ensure step.prompt exists and is valid
        if (!step || !step.prompt || typeof step.prompt !== 'string') {

            // Fallback: return empty string to prevent inserting wrong data
            return '';
        }
    ...
    async sendPromptToAI(prompt, stepIndex) {
        ...
        if (stepIndex === 0) {
            ...
            showNotification(`ℹ️ Running in simulation mode on ${platformName}. Visit claude.ai, chatgpt.com, chat.deepseek.com, gemini.google.com, or lovable.dev for real execution.`, 'info');
        }
    ...
    async sendToClaude(prompt, stepIndex) {
        // ✅ DEFENSIVE: Validate prompt before insertion
        if (!prompt || typeof prompt !== 'string' || prompt.trim() === '') {

            throw new Error(`Invalid prompt for step ${stepIndex + 1}`);
        }
    ...
    stop() {
        ...
        // Reset buttons: hide minimize, show copy
        const copyBtn = shadowRoot.querySelector('#copy-btn');
        const minimizeBtn = shadowRoot.querySelector('#minimize-btn');
        if (copyBtn) copyBtn.style.display = 'block';
        if (minimizeBtn) minimizeBtn.style.display = 'none';

        // If minimized, restore and remove minimized state
...
function displayWorkflowSteps(workflow) {
    ...
    workflow.steps.forEach(step => {
        // ✅ DEFENSIVE: Validate step has required properties
        if (!step || !step.prompt || typeof step.prompt !== 'string') {

            return; // Skip this invalid step
        }
    ...
    if (isBuiltIn) {
        ...
        if (hasVariables) {
            actionBtn.innerHTML = `
                ...
                <span style="display: block; line-height: 1 !important;">Add Variables</span>
            `;
            ...
            actionBtn.innerHTML = `
                ...
                <span style="display: block; line-height: 1 !important;">Create Editable Copy</span>
            `;
        }
...
// Store executor globally for state restoration
activeWorkflowExecutor = executor;
...
closeBtn.addEventListener('click', () => {
    // If workflow is executing, minimize instead of closing
    if (isWorkflowExecuting && executor.isRunning) {
        minimizeWorkflowUI();
        return;
    }
...
function showVariableInputForm(workflow) {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        showSuccessConfirmation(formContainer, workflowWithValues);
    });
...
function showSuccessConfirmation(formContainer, workflowWithValues) {
    ...
    confirmSection.id = 'success-confirmation-section';
    ...
    // Title
    const title = document.createElement('h3');
    title.textContent = 'Successfully Saved!';
    title.style.cssText = `
    ...
    description.textContent = 'Workflow has been saved to Custom templates folder.';
    ...
    okBtn.addEventListener('mouseleave', () => {
        okBtn.style.background = 'rgba(255, 215, 0, 0.1)';
    });
...
// ========== INLINE SAVE CONFIRMATION (OLD - KEPT FOR REFERENCE) ==========
...
function initializeUIHandlers() {
    ...
    if (!isDragging) {
        // Check if workflow is minimized and restore it
        if (isWorkflowMinimized) {
            maximizeWorkflowUI();
            return;
        }

        // Prevent processing if user just saved a prompt with Alt+S
        if (justSavedPrompt) {
            showNotification('⏸️ Prompt saved! Wait a moment before processing.', 'warning');
            return;
        }
        ...
        // ✅ FIXED: Always just open extension on icon click, never process text
        // All text transformation features have been removed - only right-click features remain
        if (lastResult && selectedMode !== 'smart_followups' && selectedMode !== 'smart_actions' && selectedMode !== 'smart_enhancements') {
            ...
            const placeholderMessages = {
                ...
                default: 'Select a mode to begin...'
            };
...
function createAuthIframe(type = 'signup') {
    return new Promise((resolve, reject) => {
        // Hide extension UI (shadow host) during auth to prevent z-index conflicts
        const shadowHost = document.getElementById('solthron-shadow-host');
        const originalShadowDisplay = shadowHost ? shadowHost.style.display : '';
        if (shadowHost) {
            shadowHost.style.display = 'none';
        }

        // Hide autocomplete dropdown if visible
        const autocomplete = document.getElementById('solthron-autocomplete');
        const originalAutocompleteDisplay = autocomplete ? autocomplete.style.display : 'none';
        if (autocomplete) {
            autocomplete.style.display = 'none';
        }

        // Hide any visible notifications
        const existingNotifications = Array.from(document.querySelectorAll('div')).filter(el => {
            return el.style.position === 'fixed' &&
                   el.style.bottom === '80px' &&
                   el.style.right === '20px' &&
                   el.style.zIndex === '999999';
        });
        existingNotifications.forEach(notif => notif.style.display = 'none');

        // Restore function
        const restoreHiddenElements = () => {
            if (shadowHost) {
                shadowHost.style.display = originalShadowDisplay;
            }
            if (autocomplete) {
                autocomplete.style.display = originalAutocompleteDisplay;
            }
            existingNotifications.forEach(notif => notif.style.display = '');
        };
        ...
        loadingOverlay.innerHTML = `
            <div style="width: 40px; height: 40px; border: 3px solid #f3f3f3; border-top: 3px solid #4285f4; border-radius: 50%; animation: spin 1s linear infinite;"></div>
            <div style="color: #424242; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Connecting to Google...</div>
        `;
        ...
        // Show iframe and hide loading overlay when iframe loads
...
document.addEventListener('keydown', async (e) => {
    if (e.altKey && e.key === 'r') {
        ...
        // ✅ Check credits before saving
        const creditCheck = await checkCredits('save_response');
        if (!creditCheck.success) {
            showNotification(creditCheck.message || 'Please login to save responses', 'error');
            return;
        }
        ...
        if (success) {
            // ✅ Deduct credits after successful save
            const deductResult = await BackendAuth.deductCredits('save_response');
            if (deductResult.success) {
                pageCredits = deductResult.remainingCredits;
            }
