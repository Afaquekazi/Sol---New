...
let isWorkflowMinimized = false; // Track if workflow UI is minimized
let activeWorkflowExecutor = null; // Store reference to active executor
let workflowUIState = null; // Store workflow UI elements for restoration
...
function handleAutocompleteKeydown(e) {
    ...
    switch (e.key) {
        case 'ArrowDown':
            e.preventDefault();
            e.stopPropagation();
            selectedAutocompleteIndex = (selectedAutocompleteIndex + 1) % autocompleteMatches.length;
            renderAutocompleteItems(autocompleteMatches);
            break;
        ...
        case 'ArrowUp':
            e.preventDefault();
            e.stopPropagation();
            selectedAutocompleteIndex = (selectedAutocompleteIndex - 1 + autocompleteMatches.length) % autocompleteMatches.length;
            renderAutocompleteItems(autocompleteMatches);
            break;

        case 'Enter':
        case 'Tab':
            if (autocompleteMatches[selectedAutocompleteIndex]) {
                e.preventDefault();
                e.stopPropagation();
                insertAutocompleteItem(autocompleteMatches[selectedAutocompleteIndex]);
            }
            ...
            const checkProgress = () => {
                try {
                    ...
                    if (responseStarted && !responseCompletionDetected) {
                        ...
                        const timeSinceResponseStarted = now - responseStartedTime;

                        if (!stopButtonVisible && inputReady && sendButtonReady && newMessageAppeared) {

                            responseCompletionDetected = true;
                            completionDetectedTime = now;
                        } else if (timeSinceResponseStarted > 10000) {
                            // Timeout fallback: assume response completed if 10+ seconds since it started
                            responseCompletionDetected = true;
                            completionDetectedTime = now;
                        }
                    }
