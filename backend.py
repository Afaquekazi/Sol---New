...
def init_analytics_db():
    ...
    conn.commit()
    conn.close()
    print("✅ Demo analytics database initialized!")
...
def get_feature_credits(mode):
    ...
    # PDF Analysis: 5 credits
    pdf_analysis_modes = ['pdf_analysis']

    # NEW: 2 credit features
    two_credit_modes = ['workflow_step', 'save_prompt', 'save_response', 'prompt_autocomplete']

    # Free Features: 0 credits
    free_modes = ['save_note', 'save_persona']
    ...
    if mode in text_processing_modes:
        ...
        return 3
    elif mode in pdf_analysis_modes:
        return 5
    elif mode in two_credit_modes:
        return 2  # NEW
    elif mode in free_modes:
        return 0
    else:
        return 6  # Default fallback (changed from 15)
...
Keep responses SHORT, CONFIDENT, and TOPIC-SPECIFIC. Max 10-12 words."""
    ...
    try:
        print(f"🤖 Starting AI analysis for: {input_text[:50]}...")
        ...
        print(f"❌ Raw AI response: {ai_response if 'ai_response' in locals() else 'No response'}")
...
def auto_suggestion():
    ...
    try:
        ...
        print(f"🤖 Generating auto-suggestion for {platform}")
...
"""
        ...
        print(f"✅ Auto-suggestion generated: {suggestion_text}")
        return jsonify(result)

    except Exception as e:
        print(f"❌ Auto-suggestion error: {e}")
        return jsonify({
...
def analyze_context():
    ...
    try:
        ...
        print(f"🤖 Analyzing context for: {input_text[:50]}...")
        ...
        print(f"✅ Context analysis complete: {analysis_result['detected_context']}")
        return jsonify(result)
...
def summarize_ai_responses():
    ...
    try:
        ...
        print(f"📝 Summarizing {len(responses)} AI responses for platform: {platform}")
    ...
    "summaries": [
        ...
        try:
            ...
            if start_idx != -1 and end_idx > start_idx:
                ...
                print(f"✅ Successfully summarized {len(result['summaries'])} responses")
                return jsonify(result)
...
def generate_context_story():
    ...
    try:
        ...
        print(f"📖 Generating context story from {len(conversation_log)} conversation entries")
    ...
    "session_summary": {{
        ...
        try:
            ...
            if start_idx != -1 and end_idx > start_idx:
                ...
                formatted_story = f"""📖 CONTEXT STORY: {story_data.get('topic', 'AI Conversation')}
...
Copy this entire context and paste it at the beginning of your next AI conversation to continue where you left off."""
                ...
                print(f"✅ Context story generated successfully: {story_data.get('topic', 'Unknown Topic')}")
                return jsonify(result)
            ...
            fallback_story = f"""📖 CONTEXT STORY: AI Conversation Summary
...
Copy this context and reference it in future conversations to maintain continuity."""
        ...
        print(f"❌ Context story generation error: {e}")
        return jsonify({
...
def magic_pill_enhance():
    ...
    try:
        ...
        print(f"🪄 Magic pill enhancement for: {input_text[:50]}...")
...
1. ACTUAL CONTENT (greeting, casual message, simple statement, test input) → Apply minimal fixing only
2. ENHANCEMENT REQUEST (clear request to improve/fix/enhance something) → Apply appropriate enhancement level

User's input text:
{input_text}

INPUT TYPE DETECTION:
- If input is a simple greeting (hello, hi, how are you, etc.) → Just fix typos/grammar, keep it as a greeting
- If input is casual conversation or test text → Minimal changes, preserve the casual nature
- If input is clearly asking to enhance/improve/fix something → Apply enhancement framework
- If input contains action words like "make this better", "improve", "fix", "create", "write" → Enhancement request
- If input is vague but seems to be asking for help with a task → Enhancement request
...
✅ Match enhancement level to user needs (light/moderate/full)
✅ Use conversational, human language that feels natural and approachable
✅ For simple requests: focus on clarity, tone, and polish with friendly guidance
✅ For moderate requests: add helpful specifics and examples in a supportive tone
✅ For complex requests: provide comprehensive details with encouraging, human-like language
✅ Include real-world examples and practical advice that feels genuinely helpful
✅ Mention common challenges in a way that's understanding, not lecturing
✅ Frame improvements as helpful suggestions rather than rigid requirements
✅ Keep it practical and actionable while sounding like friendly expert advice
✅ Create prompts that sound like they're coming from a knowledgeable colleague
✅ Help users ask better questions in a supportive, encouraging way
✅ PRESERVE the original meaning and context exactly
❌ No robotic or overly technical language
❌ No overwhelming jargon or sterile instructions
❌ Don't sound like a manual or documentation

EXAMPLES:

ACTUAL CONTENT (minimal fixing):
❌ Input: "Hellop - how re you"
❌ Wrong Output: "Polish the greeting for tone and clarity—make it sound friendly and natural"
✅ Correct Output: "Hello - how are you?"

❌ Input: "thanks for the help"
❌ Wrong Output: "Create a more detailed thank you message with specific appreciation"
✅ Correct Output: "Thanks for the help!"

ENHANCEMENT REQUESTS (human-like, polished tone):
✅ Input: "Make this greeting more professional: Hello there"
✅ Output: "Help me make this greeting sound more professional and polished. I'd like it to use proper business etiquette, include appropriate titles if I know them, and strike the right formal tone: Hello there"
...
Give ONLY the result as output - don't explain your reasoning or add meta-commentary."""
        ...
        print(f"✅ Magic pill enhancement complete")
        return jsonify(result)

    except Exception as e:
        print(f"❌ Magic pill enhancement error: {e}")
        return jsonify({
...
def gmail_enhance():
    ...
    try:
        ...
        print(f"📧 Gmail enhancement for: {email_text[:50]}...")
        print(f"📧 Context: Reply={context.get('isReply')}, Recipients={len(context.get('recipients', []))}")
...
❌ WRONG: "Hi [Name], can we meet tomorrow..." (DON'T add placeholders!)
...
❌ WRONG: "Hi [Name], I need the reports..." (DON'T add things not there!)
...
✅ Notice: Just polished what was there, didn't add new greetings or names

WHAT TO DO:
✅ Fix grammar, spelling, punctuation
✅ Improve flow between sentences
✅ Add natural transitions ("and", "so", "because")
✅ Polish tone to be professional yet warm
✅ Make choppy sentences smoother
✅ Clarify ambiguous phrases

WHAT NOT TO DO:
❌ Don't add greetings if not present
❌ Don't add names in brackets [Name]
❌ Don't add information not implied
❌ Don't change the meaning
❌ Don't make it sound like a different person wrote it
❌ Don't over-formalize casual emails
...
Provide ONLY the reframed email text - no explanations."""
        ...
        print(f"✅ Gmail enhancement complete - {len(email_text)} → {len(enhanced_email)} chars")
        return jsonify(result)

    except Exception as e:
        print(f"❌ Gmail enhancement error: {e}")
        return jsonify({
...
def gmail_enhance_smart():
    ...
    try:
        ...
        print(f"📧 Gmail enhancement - Mode: {mode}, Blend: {blend_ratio}, Temp: {temperature}")
        print(f"📧 Context: Reply={context.get('isReply')}, Recipients={len(context.get('recipients', []))}")
...
---------|---------------------------------|--------------------------------|------------------
POLISH   | • Fix objective textual errors  | • DON'T restructure sentences  | MINIMAL — objective fixes only
         |   (spelling, grammar, punctuation, capitalization)
         | • Preserve greetings, signatures, quoted text, and lists exactly
         | • Keep contractions exactly as in original
         |                                 | • DON'T add or remove facts     |
---------|---------------------------------|--------------------------------|------------------
BALANCED | • Fix all errors and improve    | • DON'T invent new facts       | MODERATE — readability + flow
         |   sentence flow and readability | • DON'T change greeting style  | (tidy structure, minor reorganization)
         | • Add paragraph breaks where    | • DON'T replace signatures     |
         |   natural; add brief courtesy   |   or remove quoted blocks      |
         |   phrases only when natural     |                                |
---------|---------------------------------|--------------------------------|------------------
REFRAME  | • Complete rewrite with balanced| • DON'T invent new facts       | MAJOR — natural professional rewrite
         |   professional-friendly tone    | • DON'T add "Subject:" lines   | (contextual structure, not robotic)
         | • Always add greeting           | • DON'T alter signature blocks |
         | • Use natural prose as default  | • DON'T force bullets on       |
         | • Add bullets/lists ONLY for 3+ |   simple 1-2 item requests     |
         |   distinct action items         | • DON'T over-formalize casual  |
         | • Smart paragraph breaks for    |   emails into corporate speak  |
         |   readability (see rules below) |                                |
         | • Add closing when making       |                                |
...
- TONE: Strike a balance between casual and professional ("professionally friendly")
  Example: "hey can u send files" → "Hi, could you send the files when you get a chance?"
...
- STRUCTURE: Adapt to email complexity, don't force one format on everything
  • Short acknowledgments (1-2 sentences) → Brief but complete with greeting
  • Simple requests (1-2 items) → Natural prose, NO bullets needed
  • Complex requests (3+ items) → Numbered lists for clarity
  • Status updates → Natural paragraphs, not forced sections

- BULLETS: Use ONLY when truly needed (3+ distinct items/action points)
  ❌ "Can you review the document and send feedback?" → Keep as prose
  ✅ "Need: slides, budget data, Q3 report, timeline" → Use numbered list

- PARAGRAPH BREAKS (CRITICAL for clean structure):
  1. Always add blank line after greeting
  2. If body has 2+ sentences covering different sub-topics → add paragraph break between topics
  3. If body has 3+ sentences total → break after every 1-2 sentences for readability
  4. Always add blank line before closing

  EXAMPLE OF GOOD PARAGRAPH BREAKS:
  ❌ BAD (cramped):
  "Hi, Just a quick update — I've completed the analysis, and the findings are looking promising. The document is already shared in Drive, and the team has been notified. Cheers!"

  ✅ GOOD (clean structure):
  "Hi,

  Just a quick update — I've completed the analysis, and the findings are looking promising.
...
- CLOSINGS: Add naturally based on email intent
  ...
  • SKIP when: Pure FYIs, simple status updates without asks, informational notes
...
(Basic fixes: capitalization, "u"→"you", "thx"→"Thanks", punctuation; structure unchanged)
...
BTW, sales look good — should we discuss strategy?"
...
I'll update you next week. We also need to discuss the mitigation plan — can we meet Friday?"
...
I'll send you a detailed update next week. In the meantime, I'd like to discuss our mitigation plan — are you available to meet on Friday?
...
1. PRESERVE TONE MARKERS (Keep the personality):
   - Keep casual greetings: "Hey" stays "Hey", "Yo" → "Hey", "Hi all" stays "Hi all"
...
4. VOICE PRESERVATION:
   ...
   - Casual can stay casual—just clearer
...
Thanks for the update. Yeah, let's go with option 2—makes sense.
...
Quick update: I finished testing and found a couple of bugs. Working on fixes now—should be done by tomorrow."
...
Output only the enhanced email, nothing else."""
            ...
            has_indian_currency = any(word in email_lower for word in ['lakhs', 'lacs', 'crores', 'rupees', 'rs', '₹', 'inr'])
            has_us_currency = any(word in email_lower for word in ['dollars', '$', 'usd'])
            has_euro = any(word in email_lower for word in ['euros', '€', 'eur'])
...
2. CONCISENESS & DIRECTNESS: Be AGGRESSIVELY concise (business/transactional email)
   ...
   - ❌ "I hope this email finds you well" → ✅ Remove entirely
   - ❌ "I hope you're doing well" → ✅ Remove entirely
   - ❌ "I'm reaching out regarding" → ✅ "This is regarding"
   - ❌ "I wanted to reach out about" → ✅ "Quick update on"
   - ❌ "I was wondering if you might be able to" → ✅ "Could you"
   - ❌ "at your earliest convenience" → ✅ "when possible" or remove
   - ❌ "if that would be possible for you" → ✅ Remove

   USE ACTIVE VOICE:
   - ❌ "errors were made" → ✅ "we made errors" or "I found errors"
   - ❌ "the report was completed" → ✅ "the team completed the report"
   - ❌ "corrections need to be implemented" → ✅ "we need to correct these"
   - ❌ "it was discovered that" → ✅ "I discovered" or "we found"

   DON'T OVER-APOLOGIZE:
   - Only apologize when YOU caused a problem directly affecting the recipient
   - ❌ "Apologies for the oversight" (in status update) → ✅ Remove
   - Status updates and internal issues don't need apologies

   BE DIRECT:
   - ❌ "it seems that" / "it appears" → ✅ Direct statement
   - ❌ "the amount that was put in while uploading was" → ✅ "the uploaded amount was"
...
- MUST use $ symbol (NOT ₹, €, or £)
- Example: $50,000 NOT ₹50,000
...
4. PARAGRAPH BREAKS (CRITICAL - Follow This Structure):
   ✓ Always add blank line after greeting
   ✓ If body has 2+ sentences covering different sub-topics → add paragraph break between topics
   ✓ If body has 3+ sentences total → break after every 1-2 sentences for readability
   ✓ Always add blank line before closing

   STUDY THIS EXAMPLE:
   ❌ BAD (Everything cramped):
   "Hi, Just a quick update — I've completed the analysis, and the findings are looking promising. The document is already shared in Drive, and the team has been notified. Cheers!"

   ✅ GOOD (Clean paragraph structure):
   "Hi,
...
5. STRUCTURE: Adapt to email type intelligently
   - 1-2 sentence acknowledgments → Keep brief but with proper spacing
   - Simple requests (1-2 items) → Natural prose with paragraph breaks if multiple topics
   - Complex requests (3+ distinct items) → Use numbered lists with proper spacing
   - Status updates → Natural paragraphs with breaks between different updates

6. BULLETS: Use ONLY when there are 3+ distinct action items or deliverables
   - ❌ "Review the deck and send feedback" → Keep as prose (it's one request)
   - ❌ "Send report and budget numbers" → Keep as prose (only 2 items)
   - ✅ "Need slides, Q3 data, design files, and approval" → Use numbered list (4 items)
...
Output only the rewritten email body with proper paragraph breaks, nothing else."""
        ...
        print(f"✅ Gmail {mode} enhancement complete - {len(email_text)} → {len(enhanced_email)} chars")
        return jsonify(result)
...
- Structure: deadline → action needed → consequence
...
def gmail_enhance_custom():
    ...
    try:
        ...
        print(f"🎯 Custom Gmail enhancement for: {email_text[:50]}...")
        print(f"🎯 Custom instruction: {custom_prompt}")
        print(f"📧 Context: Reply={context.get('isReply')}, Recipients={len(context.get('recipients', []))}")
...
WHAT TO DO:
✅ Apply the custom instruction faithfully
✅ Keep all actual names and details
✅ Preserve the email's purpose
✅ Make natural improvements that align with the instruction
✅ Fix grammar if needed (unless instruction says otherwise)

WHAT NOT TO DO:
❌ Don't add placeholder text like [Name], [Date], etc.
❌ Don't add greetings if instruction doesn't require them
❌ Don't add information not in the original
❌ Don't change the meaning unless instruction explicitly requires it
❌ Don't ignore the user's instruction
❌ Don't add signature blocks or quoted text
...
Provide ONLY the enhanced email text - no explanations, no meta-commentary."""
        ...
        print(f"✅ Custom Gmail enhancement complete - {len(email_text)} → {len(enhanced_email)} chars")
        return jsonify(result)

    except Exception as e:
        print(f"❌ Custom Gmail enhancement error: {e}")
        return jsonify({
...
def ai_platform_custom():
    ...
    try:
        ...
        print(f"📝 Enhancement: {enhancement_instruction}")
...
Just give me the refined prompt, nothing else."""
        ...
        print(f"✅ Prompt enhanced: {len(original_prompt)} → {len(enhanced_prompt)} chars")
        print(f"📊 Enhancement preview: {enhanced_prompt[:100]}...")
...
✗ Simply agreeing to AI's follow-up questions ("Yes, do that", "Sure, create that")
✗ Vague requests ("tell me more", "explain further")
✗ Theoretical discussions without practical application
✗ Repeating what the AI already offered
✗ Any response starting with "Yes," "Sure," "Okay," regarding the AI's question

GOOD EXAMPLES based on different contexts:
- If AI provided a solution → "Show me how to integrate this with [specific technology/framework]"
- If AI explained a concept → "Create a working example that I can run locally with error handling"
- If AI gave options → "Build a comparison table with performance metrics and implementation complexity"
- If AI wrote code → "Add unit tests for edge cases and a README with setup instructions"
- If AI created content → "Convert this into a production-ready version with SEO optimization"
- If AI made a video script → "Create a detailed shot list with camera angles and transition effects"
- If AI analyzed something → "Generate an actionable checklist with priority levels and timelines"
...
Remember: The user should think "That's exactly what I need to make this real!" not "Why is it just agreeing?"."""
        ...
        try:
            ...
            logging.info(f"✅ Practical auto action generated (score: {quality_score:.2f}): {prompt_text[:50]}...")
            return jsonify(result)
...
def demo_dashboard():
    ...
    <body>
        <h1>📊 Solthron Demo Analytics Dashboard</h1>
        ...
        <div class="metrics">
            <div class="card">
                <div class="label"><span class="user-icon">👥</span> Unique Users</div>
            ...
            <div class="card">
                <div class="label"><span class="pill-icon">💊</span> Pill Clicks</div>
